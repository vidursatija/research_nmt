...
class DoubleRNNModel():
	...
	def __init__(self, vocab_size, hidden_size=64, learn_rate=0.25, encoding_mode=ModelMode.ENCODE, mid_state=None):
		...
		if encoding_mode == ModelMode.DECODE:
			...
			ones = tf.constant(1, shape=[vocab_size, hidden_size], dtype=tf.float16, name='e_ones')
			mul_val = tf.reshape(tf.nn.embedding_lookup(ones, self.inputX), [batch_size, -1, self.hidden_units])
		...
		with tf.variable_scope("RNN") as scope:
			...
			if encoding_mode == ModelMode.DECODE:
				x = tf.reshape(mid_state, [batch_size, 1, self.hidden_units])*mul_val
...
class TranslatorModel():
	...
	def __init__(self, vocab_size_from, vocab_size_to, hidden_size=64, learn_rate=0.01):
		...
		self.new_state = tf.matmul(tf.tanh(tf.matmul(self.encoder_model.f_state, translator_w) + translator_b), reg_w) + reg_b
		...
		tvars = tf.trainable_variables()#[translator_w, translator_b, reg_w, reg_b]#tf.trainable_variables()
		
		optimizer = tf.train.AdadeltaOptimizer(learn_r)
		gradsvars = optimizer.compute_gradients(self.cost, tvars)
		#print(gradsvars)
		grads, _ = tf.clip_by_global_norm([g for g, v in gradsvars], 5)#tf.clip_by_global_norm(tf.gradients(cost, tvars), 10)
	...
	def run_n_epochs(self, sess, train_writer, inputX, inputY, n_files, n=1, epoch_num=0):
		avg_err = 0.0
		for e in range(n):
			avg_err = 0.0
			for f in range(n_files):
				summary = None
				if f%150 == 149:
					summary, _ = sess.run([self.merged, self.train_op], feed_dict={self.encoder_model.inputX: inputX[f], self.decoder_model.inputX: inputY[f], self.targets: inputY[f]})
					train_writer.add_summary(summary, epoch_num*n_files + f)
			...
			train_writer.add_summary(summary, epoch_num*n_files + n_files-1)
			tf.logging.info(" ".join([str(avg_err), "at major epoch", str(epoch_num)]))
			#self.learn_rate = self.learn_rate*0.99
