...
class DoubleRNNModel():
	...
	def __init__(self, vocab_size, hidden_size=64, learn_rate=0.25, encoding_mode=ModelMode.TRAIN):
		...
		with tf.variable_scope("RNN") as scope:
			...
			if encoding_mode == ModelMode.DECODE:
				outputs_2, f_state_2 = tf.nn.dynamic_rnn(gru_cell, x, initial_state=self.mid_state, dtype=tf.float16)
...
class TranslatorModel():
	...
	def __init__(self, vocab_size_from, vocab_size_to, from_save_path, to_save_path, hidden_size=64, learn_rate=0.01):
		...
		with tf.name_scope("decode_net"):
			self.decoder_model = DoubleRNNModel(vocab_size_to, hidden_size=hidden_size, encoding_mode=ModelMode.DECODE)
		...
		tf.assign(self.decoder_model.mid_state, tf.tanh(tf.matmul(self.encoder_model.f_state, translator_w) + translator_b)) 
